...
NFS share goes down
        |
        v
Zabbix detects trigger on client (pve1/pve2)
        |
        v
Action 29 fires:
...
Script creates trigger file: /etc/zabbix/scripts/trigger_recovery
        |
        v
systemd path unit detects trigger file
        |
        v
systemd service runs /mnt/scripts/nfs_recovery.sh on the HOST
        |
        v
Recovery script uses midclt to re-enable disabled shares
        |
        v
...
Runs inside the Zabbix agent container. Creates a trigger file to signal the host.
...
Watches for the trigger file created by the Zabbix agent.
...
2. Action 29 fires: one Telegram message to yair.
3. Remote command runs on both TrueNAS hosts.
4. Agent creates trigger file.
5. systemd detects file and runs recovery.
6. Disabled share is re-enabled via midclt.
7. Zabbix detects recovery and closes the problem.
